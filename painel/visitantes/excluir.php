...
//Valida autorização para exclusão
...
$consultar = $pdo->prepare("SELECT * FROM visitantes WHERE id = :id");
...
if ($total_reg > 0) {
    // Excluir o visitante
    $excluir = $pdo->prepare("DELETE FROM visitantes WHERE id = :id");
    $excluir->bindValue(':id', $id);
    $excluir->execute();
    
    echo "<script>window.alert('Visitante excluído com sucesso!')</script>";
    echo "<script>window.location='../index.php?pagina=visitantes'</script>";
} else {
    echo "<script>window.alert('Visitante não encontrado!')</script>";
    echo "<script>window.location='../index.php?pagina=visitantes'</script>";
}
